...
def runQuery(conn):
    select_Query = "SELECT student_id FROM student_counseling_information WHERE department_admission= (SELECT department_id FROM department_information WHERE doe= (SELECT MIN(doe) FROM department_information));"
